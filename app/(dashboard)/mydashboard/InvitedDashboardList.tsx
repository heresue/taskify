import { RefObject } from 'react';
// import { Invitation } from '@/types';
import Button from '@/components/common/Button';
import { Invitation } from './types';

interface Props {
  invitations: Invitation[];
  onAccept: (id: number) => void;
  onReject: (id: number) => void;
  isLoading: boolean;
  scrollRef: RefObject<HTMLDivElement | null>;
}

const InvitedDashboardList = ({ invitations, onAccept, onReject, isLoading, scrollRef }: Props) => {
  return (
    <div className="w-full overflow-hidden rounded-lg">
      {/* Header */}
      <div className="text-gray400 text-regular16 grid grid-cols-3 px-[76px]">
        <span>이름</span>
        <span>초대자</span>
        <span className="text-center">수락 여부</span>
      </div>

      {/* Body */}
      <div
        ref={scrollRef}
        className="border-gray200 border-b max-h-[390px] divide-y overflow-y-auto border-t"
      >
        {invitations.map((invitation) => (
          <div key={invitation.id} className="grid grid-cols-3 items-center px-[76px] py-[23px]">
            <span>{invitation.dashboard.title}</span>
            <span>{invitation.inviter.nickname}</span>
            <span className="flex justify-center gap-2">
              <Button onClick={() => onAccept(invitation.id)} disabled={isLoading}>
                수락
              </Button>
              <Button
                variant="outline"
                onClick={() => onReject(invitation.id)}
                disabled={isLoading}
              >
                거절
              </Button>
            </span>
          </div>
        ))}
      </div>
    </div>
  );
};

export default InvitedDashboardList;
